...
def _classification_stats(pairs: list[tuple[bool, float]], threshold: float) -> dict[str, float | None]:
    failures = [score for label, score in pairs if label]
    non_failures = [score for label, score in pairs if not label]
    if not failures or not non_failures:
        return {
...
def _recommend_threshold(pairs: list[tuple[bool, float]], fallback: float) -> dict[str, float | None]:
    failures = [score for label, score in pairs if label]
    non_failures = [score for label, score in pairs if not label]
    if not failures or not non_failures:
        return {
    ...
    best_key = (
        ...
        -(best_stats['false_positive_rate'] if best_stats['false_positive_rate'] is not None else 1.0),
...
def summarize(prefixes: list[dict], baselines: list[dict], judge: list[dict], thresholds: dict[str, float] | None = None) -> dict:
    ...
    calibration = {
        'judge': _recommend_threshold(overall['_judge_pairs'], chosen_thresholds['judge_failure_threshold']),
        'progress': {
            'recommended_threshold': round(float(1.0 - chosen_thresholds['progress_failure_threshold']), 6),
            'hit_rate': None,
            'false_positive_rate': None,
            'balanced_accuracy': None,
    ...
    chosen_thresholds['judge_failure_threshold'] = float(calibration['judge']['recommended_threshold'])

    for row in rows:
        judge_predicts_failure = row['judge_score'] >= chosen_thresholds['judge_failure_threshold']
        progress_predicts_failure = row['progress_failure_score'] >= (1.0 - chosen_thresholds['progress_failure_threshold'])
    ...
    return {
        'thresholds': {
            'judge_failure_threshold': round(float(chosen_thresholds['judge_failure_threshold']), 6),
            'progress_failure_threshold': round(float(chosen_thresholds['progress_failure_threshold']), 6),
        },
