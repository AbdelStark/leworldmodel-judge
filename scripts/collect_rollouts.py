...
def _synthetic_family_state(policy_family: str, t: int, horizon: int) -> tuple[float, float, float, float, float, float]:
    frac = (t + 1) / horizon
...
def collect_synthetic(task: str, episodes: int, policy_family: str = 'random') -> list[dict]:
    ...
    for ep in range(episodes):
        ...
        for t in range(horizon):
            progress, in_place, grasp, success_signal, unscaled_reward, obs_progress = _synthetic_family_state(policy_family, t, horizon)
            reward = 1.0 if success_episode and t == horizon - 1 else (-0.1 if policy_family == 'random' and t > horizon // 2 else 0.0)
            step = RolloutStep(
...
def collect_metaworld(task: str, episodes: int, max_steps: int | None, seed: int, policy_family: str = 'random') -> list[dict]:
    ...
    for ep in range(episodes):
        ...
        for t in range(horizon):
            ...
            step_payload: dict[str, float | str] = {'source': 'metaworld', 'policy_family': policy_family}
...
def main() -> None:
    ...
    parser.add_argument('--policy-family', default='random', help='random, expert, weak, doomed, misleading, or comma-separated list')
    ...
    for family_index, policy_family in enumerate(policy_families):
        if policy_family not in SYNTHETIC_FAMILIES:
            raise SystemExit(f'policy-family must be one of {SYNTHETIC_FAMILIES}; got {policy_family}')
        for offset, task in enumerate(tasks):
            task_seed = args.seed + family_index * 100 + offset
            if args.source == 'metaworld':
                rows.extend(collect_metaworld(task, args.episodes, args.max_steps, task_seed, policy_family=policy_family))
            else:
                rows.extend(collect_synthetic(task, args.episodes, policy_family=policy_family))

    write_jsonl(args.output, rows)
    print(f'wrote {len(rows)} rollout steps for tasks={tasks} policy_families={policy_families} to {Path(args.output)}')
